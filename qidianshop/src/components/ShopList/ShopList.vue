    ...
    <li class="shop_li border-1px" v-for="(item,index) in shoplists" :key="index"> 
      ...
      <div class="shop_right"> 
       ...
       <section class="shop_distance"> 
        <p class="shop_delivery_msg"> <span>&yen;20起送</span> <span class="segmentation">/</span> <span>配送费约&yen;{{item.fee}}</span> </p> 
       </section> 
      </div> </router-link>
    </li> 
   </ul> 
...
<script>
//导入api下的index.js
//import {shoplists} from '../../api/index'
...
export default {
  ...
  //异步获取
  /*async mounted(){
    const slist=await shoplists();
    console.log(slist);
    //给shoplists变量赋值
    this.shoplists=slist;
